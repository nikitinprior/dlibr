...
char		msg_425e[]  = "Usage: libr [-w][-pwidth] key [subkeys symbol] file.lib [modules ...]";
char 		arry_42f0[] = { 0 };
...
int	sub_013dh(char *, char *);		//  1 ok++   |  sub_12d4h Delete, Extract and List modules with symbols
...
int	sub_0366h(char *, uint, uint, FILE *);	// 11 ok++   |  sub_02cch Delete modules
...
void sub_025bh( void (*fun)(char *, uint) ) {
    ...
    l1 = 0;
    while(l1 != num_ofiles) {
	if(word_4749[l1] == 0) fun(word_4747[l1], l1);
	++l1;
    }
}
...
void sub_02e5h() {
    ...
    if(num_ofiles == 0) fatal_err("delete what ?");
    sub_0487h();
    sub_034ah();
    sub_0621h(sub_02b3h);
...
void sub_0317h(char * p1, long dummy) {
    ...
    if(sub_01fbh(p1) != 0) sub_0912h(p1);
}
...
int sub_0366h(char * p1, uint p2, uint p3, FILE * p4) {
	...
	fatal_err("Write error on %s file", (p4 == fptr_4967) ? "temp" : "output");
    }
    return p2;
}
...
void sub_03dah(char * p1) {
    ...
    if(st == 0) goto m2;
    if(strcmp(st, ".lib") == 0)  goto m3;
    if(strcmp(st, ".LIB") == 0)  goto m3;

m2: warning("library file names should have .lib extension: %s", fname_4758);

m3: word_4752   = 0;
    num_modules = 0;
    return;

m5: if(read1(buff_4969, 4, 1, fptr_4761) != 1) unexp_eof();
...
void sub_0487h() {
    ...
    if(fptr_4761 == 0) return;
    if((fptr_4b6d = fopen(fname_4758, "rb")) != 0) {
	if(fseek(fptr_4b6d, ((long)word_4752+4), SEEK_SET) != -1) return;
    }
    err_open(fname_4758);
}
...
void sub_053ch() {
    ...
    if((fptr_4967 = fopen("libtmp.$$$", "rb")) == 0)  err_open("libtmp.$$$");
    if((fptr_4761 = fopen(fname_4758, "wb")) == 0)    err_open(fname_4758);
...
void sub_0621h( void (*funptr)(char *, long) ) {
    ...
    loc = num_modules;
    while(loc != 0) {
	if(read1(buff_4969, 12, 1, fptr_4761) != 1) unexp_eof();
...
void sub_0727h(void (*funptr)(char *, int)) {
    ...
    l1 = word_475f;
    while(l1 != 0) {
	if((l2 = fgetc(fptr_4761)) == -1) unexp_eof();
	sub_0cb3h(buff_4767);
	funptr(buff_4767, l2);
	--l1;
    }
...
void sub_0789h() {
	...
	if(read1(buff_4767, 1, l2, fptr_4761) != l2) unexp_eof();
...
void sub_0874h() {
    ...
    while(l1 != 0) {
	l2 = (l1 < 0x200) ? l1 : 0x200;
	if(read1(buff_4767, 1, l2, fptr_4b6d) != l2) unexp_eof();
	sub_0366h(buff_4767, 1, l2, fptr_4967);
	word_474d += l2;
	l1 -= l2;
    }
...
void sub_0912h(char * p1) {
    ...
    if((st = fopen(p1, "wb")) == 0) err_open(p1);
    l1 = long_4763;
    while(l1 != 0) {
	l2 = (l1 < 0x200) ? l1 : 0x200;
	if(read1(buff_4767, 1, l2, fptr_4b6d) != l2) unexp_eof();
...
void sub_09cdh(char * p1, uint p2) {
    ...
    if((st = fopen(p1, "rb")) == 0) err_open(p1);
    l3  = 0;
    l1  = word_474b[p2];
...
void sub_0cb3h(register char * p1) {
    ...
    do {
	if((l1 = fgetc(fptr_4761)) == -1) unexp_eof();
	*(p1++) = l1;
    } while(l1 != 0);
}
...
void sub_0d22h(char * p1, int p2) {
    ...
    if((((char)p2 == 0) ? byte_4b72 : byte_4b73) == 0) return;
    if(strcmp(word_4b74, p1) != 0) {
	if(*p1 != '_') return;
	if(strcmp(word_4b74, p1+1) != 0) return;
    }
    byte_4b76 = 1;
    byte_4b71 = p2;
}
...
void sub_0d7ch(char * p1, long dummy) {
    ...
    if(sub_01fbh(p1) == 0) return;
    if(word_4b74 != 0) {
	byte_4b76 = 0;
	sub_0727h(sub_0d22h);
	if(byte_4b76 == 0) return;
    }
    printf("%-15.15s", p1);
    if(word_4b74 != 0)
...
void sub_0df2h(char * p1, char * p2) {
	...
	do {
	    switch(*p1) {
	      case 'd':	byte_4b72 = 1; break;
	      case 'u':	byte_4b73 = 1; break;
	      default:	fatal_err("Subkeys: d(defined) u(ndefined)");
	    }
	} while(*(++p1) != 0);
    }
...
void sub_0ec1h(char * p1, long dummy) {
    ...
    if(word_4b79 == 0) return;
    if(word_4b79 >= word_4b77) return;
    putchar('\n');
}
...
void sub_0f0dh() {
    ...
    if((word_4b77 = (width - 16)/16) == 0) word_4b77 = 1;
...
m7: fatal_err(msg_425e); // "Usage: ...]"
    goto m4;

m8: ban_warn = 1;		// Disable warning messages 
    goto m4;

m9: if(isdigit(*l1) == 0) goto m7;
...
//----------------- The code below this line generates the correct binary code.

m11: if(argc < 2) fatal_err(msg_425e); // "Usage: ...]"
    l2 = *argv;
    --argc;	
    ++argv;
    ...
    if(argc == 0) fatal_err(msg_425e); // "Usage: ...]"

    sub_03dah(*argv);
    sub_01cch(argc-1, argv+1);

    (func_42e1[word_4b7e])(l2+1, l1);  // Execute a function depending on the key 

    sub_12e8h(err_num != 0);	// Terminate the program with the appropriate error code
}
...
char * allocmem(int p1) {
    ...
    if((l1 = malloc(p1)) == 0) fatal_err("Cannot get memory");
...
int calc_val(uchar * p1) {
    ...
    return (uint)(*((uint)order16[1]+p1) << 8) + (uint)*((uint)order16[0]+p1);
}
...
void sub_1457h(uchar * p1, int p2) {
    ...
    st = word_4d84;
    if((st->i2 = p2) != 6) byte_4d8a = 1;
    strcpy((word_4d84->i1 = allocmem(strlen((char *)p1)+1)), (char *)p1);

    ++word_4d84;
}

// Object file record types: | Length (16 bits)     | Record type (8 bits) | Data (Length*8 bits) |
...
int sub_14aeh(char * p1) {
    ...
    if(fptr_4d8b == 0) err_open(fname_5369);
    byte_4d8a = 0;
    long_4d86 = 0;
    word_4d84 = &word_4d8d;
...
m2:
    ...
    if(recd_type == SYM_RECORD)   goto m3;
    if(recd_type == END_RECORD)   goto m5;
    if(recd_type == IDENT_RECORD) goto m4;
    sub_15fbh();
    goto m2;

m3: sub_162ah();	// SYM_RECORD
    goto m2;

m4: sub_1351h();	// IDENT_RECORD
    goto m2;

m5: fclose(fptr_4d8b);	// END_RECORD

    if((ulong)long_4d86 >= (ulong)0x00010000) fatal_err("file too big");
    if(byte_4d8a == 0) warning("module %s defines no symbols", fname_5369);
    return long_4d86;
/*
	switch(recd_type) {
...
void sub_1568h() {
    ...
    if(recd_type == 0) goto m2;
    if(recd_type < 9)  goto m3;

m2: bf_format(fname_5369, "unknown record type: %d", (uint)recd_type);
//	509 = 512-3
m3: if(0x01FD < (length = conv_atoi1(buff_4b81)))
...
void sub_15fbh() {
    ...
    if(fseek(fptr_4d8b, length, SEEK_CUR) != -1) return;
...
void sub_162ah() {
	...
	if(l3 == 2) goto m5;			// m4:
	if(l3 == 6) {
m5:	    sub_1457h((uchar *)l1, (uchar)l3);
	}
	st = l1;				// m6:
...
void sub_175ah() {
    ...
    if(num_ofiles == 0) fatal_err("replace what ?");
    sub_0487h();
    sub_034ah();
    sub_0621h(sub_16e8h);
    sub_025bh(sub_0a93h);
